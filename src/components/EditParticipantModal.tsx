...
export default function EditParticipantModal({
  ...
  const [gradeLevel, setGradeLevel] = useState(participant.gradeLevel);
  const [email, setEmail] = useState(participant.email);
  const [dietaryRestriction, setDietaryRestriction] = useState(participant.dietaryRestriction);
  const [teamName, setTeamName] = useState(participant.teamName);
  const [customTeam, setCustomTeam] = useState(!existingTeams.includes(participant.teamName));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await onSave(participant._id, { name, gradeLevel, email, dietaryRestriction, teamName });
      onClose();
    } catch (err: unknown) {
        ...
        <div className="relative bg-black/90 border border-white/10 rounded-2xl p-6 backdrop-blur-xl shadow-2xl">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-semibold text-white">Edit Participant</h2>
            </div>
            <button
              onClick={onClose}
